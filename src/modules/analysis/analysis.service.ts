...
export class AnalysisService {
  ...
  async queryProposal(dao): Promise<void> {
    ...
    response.data.data.proposals.forEach(
      (item) => {
  ...
  async queryBalance(dao): Promise<void> {
    ...
    const res = await this.covalenthqService.getTokenBalancesForAddress(address);
    ...
    res.data.items.forEach(
      (item) => {
        ...
        try {
          ...
          if (old_balance) {
            const diff_usd = (balance - old_balance) * parseFloat(item.quote_rate)
            const diff_percent = (balance - old_balance) / old_balance
            if (diff_percent >= percent_threshold) {
              const msg = `${(balance - old_balance) / denominator} #${item.contract_ticker_symbol} (${diff_usd} USD)\n has been transferred to #${dao}`
              this.logger.debug(msg)
              // this.tweetService.tweet(msg)
            } else if (diff_percent < -percent_threshold) {
              const msg = `${(old_balance - balance) / denominator} #${item.contract_ticker_symbol} (${-diff_usd} USD)\n has been transferred to #${dao}`
              this.logger.debug(msg)
              // this.tweetService.tweet(msg)
            }
          }
